...
class UserSavePage():
    ...
    def get_save_user_field(self):
        ...
        try:
            ...
            time.sleep(2)
            popup_wrapper = popup_list.wrapper_object()
        except TimeoutError:
            raise Exception("고객등록 팝업을 찾을 수 없습니다.")

        popup_wrapper.children()[0].set_focus()
        time.sleep(1)
        self.user_save_popup_field = popup_wrapper.children()[0]
    ...
    def input_jno_info(self, edit_list, jno):
        if jno:
            ElementFinder.input_text(edit_list[0], jno[7:14])
            ElementFinder.input_text(edit_list[1], jno[0:6])
        else:
